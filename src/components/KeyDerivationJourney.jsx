...
function KeyDerivationJourney() {
  const [entropyBinary, setEntropyBinary] = useState('');
  ...
  const entropy = useMemo(() => {
    if (entropyBinary.length !== 128) return "";
    let hex = '';
  ...
  const generateEntropy = () => {
    ...
    const bin = Array.from(arr).map(byte => byte.toString(2).padStart(8, '0')).join('');
    setEntropyBinary(bin);
    setStep(2);
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-2">
              ...
              <textarea 
                ...
                placeholder="Paste or type 128 bits of 0s and 1s, or click Generate Random..."
                className={`font-mono text-[10px] md:text-xs p-6 bg-gray-50 dark:bg-gray-900/80 rounded-3xl break-all border-2 leading-relaxed shadow-inner min-h-[80px] w-full outline-none transition-all resize-none ${
                    entropyBinary && (entropyBinary.length === 128) 
              ...
              />
              <div className="flex justify-end text-xs text-gray-400 font-mono mt-1">
                {entropyBinary.length} / 128 bits
              </div>
            </div>
...
export default KeyDerivationJourney;
